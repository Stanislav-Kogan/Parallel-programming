...
void lu_decomposition(double **A, int n) {
    int i, j, k;
    #pragma omp parallel for private(i, j, k) shared(A, n) schedule(dynamic)
    for (k = 0; k < n; k++) {
        for (i = k + 1; i < n; i++) {
            A[i][k] /= A[k][k];
        }
...
void forward_substitution(double **L, double *b, double *y, int n) {
    int i, j;
    #pragma omp parallel for private(i, j) shared(L, b, y, n) schedule(dynamic)
    for (i = 0; i < n; i++) {
        y[i] = b[i];
        for (j = 0; j < i; j++) {
            y[i] -= L[i][j] * y[j];
...
void backward_substitution(double **U, double *y, double *x, int n) {
    int i, j;
    #pragma omp parallel for private(i, j) shared(U, y, x, n) schedule(dynamic)
    for (i = n - 1; i >= 0; i--) {
        x[i] = y[i];
        for (j = i + 1; j < n; j++) {
            x[i] -= U[i][j] * x[j];
...
int main(int argc, char *argv[]) {
    ...
    FILE *fp;
    fp = fopen("output_C_openMP.txt", "w");
    if (fp == NULL) {
